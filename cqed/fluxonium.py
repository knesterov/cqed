...
########################################################################
"""The Fluxonium class for representing superconducting fluxonium
qubits.
"""
...
class Fluxonium(object):
    ...
    The calculations are based on expressing the Hamiltonian in the
    harmonic-oscillator basis generated by the capacitive and inductive
    terms (LC basis). May be impractical in certain limits such as
    the blochnium limit, when the inductive term is very small.

    Parameters
    ----------
    E_L : float
        Qubit inductive energy.
    E_C : float
        Qubit capacitive energy.
    E_J : float
        Qubit Josephson energy.
    phi_ext : float, default=np.pi
        Static external dimensionless magnetic flux.
    nlev : int, default=5
        The number of qubit eigenstates to calculate/return.
    nlev_lc : int, default=30
        The number of levels in the truncated harmonic-oscillator basis
        formed by the capacitive and inductive terms.

    Notes
    -----
    The Hamiltonian has the form
    .. math::
    H = 4 E_C n^2 + (1/2) E_L \phi^2 - E_J \cos(\phi - \phi_{ext}),
    where .. math::`phi` and .. math::`n` are dimensionless flux
    ...
    def __init__(self, E_L, E_C, E_J, phi_ext=np.pi,
        ...
        self.phi_ext = phi_ext  # The external dimensionless flux.
        self.nlev = nlev  # The number of qubit eigenstates.
        self.nlev_lc = nlev_lc  # The number of states before diagonalization.
        self.type = 'qubit'

    def __str__(self):
        s = ('A fluxonium qubit with E_L = {} '.format(self.E_L)
             + ', E_C = {} '.format(self.E_C)
             + ', and E_J = {} '.format(self.E_J)
    ...
    def levels(self, nlev=None):
        ...
        ----------
        nlev : int, optional
            The number of eigenvalues if different from `self.nlev`.
    ...
    def potential(self, phi_points=None):
        ...
        phi_points : float or ndarray, optional
            Value(s) of the dimensionless flux variable.
    ...
    def wavefunc(self, level_ind, phi_points=None):
        ...
        def ho_wf(phi, k, E_C, E_L):
            # Calcultes wave functions of the harmonic oscillator.
            ratio = (8.0 * E_C / E_L)**(0.25)
            coeff = (2.0**k * np.math.factorial(k)
                     * np.sqrt(np.pi) * ratio)**(-0.5)
            return (
                coeff * np.exp(-0.5 * (phi / ratio)**2) * hpoly(k, phi/ratio))
